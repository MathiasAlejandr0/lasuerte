...
function seedAffiliates(): DbAffiliate[] {
  ...
  const password_hash = hashPassword(demoAffiliatePassword());
  return [
    {
      id: "aff-stjp48",
      code: "STJP48",
      name: "Embajador Sur",
      email: "embajador@suertu2s.cl",
      phone: null,
    ...
    },
    {
      ...
      code: "DEMO01",
      name: "Vendedor Demo",
      email: "demo@suertu2s.cl",
      phone: null,
      commission_type: "fixed",
      commission_value: 1000,
      active: true,
      notes: "Comisión fija $1.000 por venta",
      password_hash,
      created_at: now,
      updated_at: now,
    },
...
/** Pedidos de demostración para previsualizar el admin (solo desarrollo). */
export function memorySeedDemoSales() {
  const s = store();
  const hasDemo = s.orders.some((o) => o.email.startsWith("demo+"));
  if (s.demoSeeded || hasDemo) {
    // Recarga en desarrollo: si faltan ítems/números, regenerar una vez
    if (
      hasDemo &&
      !s.items.some((i) =>
        s.orders.some(
          (o) => o.email.startsWith("demo+") && o.id === i.order_id,
        ),
      )
    ) {
      s.orders = s.orders.filter((o) => !o.email.startsWith("demo+"));
      s.payouts = [];
      s.demoSeeded = false;
    } else {
      s.demoSeeded = true;
  ...
  const aff1 = memoryFindAffiliateByCode("STJP48")!;
  const aff2 = memoryFindAffiliateByCode("DEMO01")!;
  const now = Date.now();
  ...
  const samples = [
    ...
    },
    {
      ...
      total: 8000,
      aff: aff1,
      email: "demo+2@suertu2s.cl",
      name: "Luis Soto",
      provider: "webpay" as PaymentProvider,
    },
    {
      daysAgo: 3,
      total: 5000,
      aff: aff2,
      email: "demo+3@suertu2s.cl",
      name: "Camila Ríos",
    ...
    },
    {
      ...
      total: 5000,
      aff: null,
      email: "demo+pending@suertu2s.cl",
      name: "Pendiente Demo",
      provider: "webpay" as PaymentProvider,
  ...
  for (const sample of samples) {
    const created = new Date(now - sample.daysAgo * 86400000).toISOString();
    ...
    const order: DbOrder = {
      ...
      phone: "+56912345678",
      status,
      payment_provider: sample.provider,
      payment_external_id:
        status === "paid" ? `demo_${orderId.slice(0, 8)}` : null,
      total_clp: sample.total,
      raffle_id: getRaffle().id,
      referral_code: sample.aff?.code ?? null,
      referral_name: sample.aff?.name ?? null,
      affiliate_id: sample.aff?.id ?? null,
      created_at: created,
      paid_at: status === "paid" ? created : null,
      confirmation_email_sent_at: status === "paid" ? created : null,
    };
    s.orders.push(order);

    const lines = packForTotal(sample.total);
    for (const line of lines) {
      s.items.push({
        ...
        unit_price_clp: line.pack.priceClp,
        ticket_count: line.pack.ticketCount * line.quantity,
      });
    }
  ...
  // sample payout for STJP48
  s.payouts.push({
    id: randomUUID(),
    affiliate_id: aff1.id,
    amount_clp: 1000,
    period_from: new Date(now - 20 * 86400000).toISOString().slice(0, 10),
    period_to: new Date(now - 10 * 86400000).toISOString().slice(0, 10),
    note: "Anticipo demo",
    paid_at: new Date(now - 7 * 86400000).toISOString(),
